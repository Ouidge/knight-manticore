---
type: adj
subtype: effet
---
> desc:: Pour utiliser cette arme, vous **devez** dépenser une (et une seule) action de déplacement **et** une (et une seule) action de combat. Les actions supplémentaires acquises ne sont pas touchées par cet effet.

> [!example] Exemple
> Le personnage de Nicolas, possédant une masse magnétique, s’est avancé vers son ennemi pour être à portée de frappe. L’arme possédant l’effet lourd, le personnage de Nicolas devra attendre le tour prochain pour attaquer son ennemi. 